...
public class RestaurantController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		if (action.equals("addBranch")) {
			...
			request.getRequestDispatcher("addRecipe.jsp").forward(request,
					response);
		} else if (action.equals("adddCuisine")) {
		...
		} else if (action.equals("viewCusine")) {
			List<Cuisine> cuisines = new CuisineDAO().getCuisines();

			request.setAttribute("cuisineList", cuisines);
			request.getRequestDispatcher("viewCuisines.jsp").forward(request,
					response);
		}
	}
	...
	protected void doPost(HttpServletRequest request,
		...
		if (isMultipart) {
				...
				for (FileItem fileItem : list) {
					...
					if (count == 0) {
						if (!fileItem.isFormField()) {
							String imagePath = "C:/Users/Sakhi/urbanspoon"
			...
			if(0<new CuisineDAO().insert(cuisine))
			{
				response.sendRedirect("RestaurantController?action=viewCusine");
			}
